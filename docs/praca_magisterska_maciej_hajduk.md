...
gdzie $TP$ to ilość wyników _True Positive_, a $FN$ to ilość wyników _False Negative_ w macierzy konfuzji.
...
3. Stworzenie pętli iterującej od $i = 1$ do $X$, gdzie $X$ to ilość cech zbioru, kolejno dla każdej metody.
...
![Procentowa ilość cech wymagana do spełniania hipotezy zerowej testu Wilcoxona dla części zbiorów danych (stopień niezbalansowania pomiędzy 1:1.5 a 1:9).](./figures/wilcoxon_feats_part_1.png){#fig:wilcoxon_feats_part_1}
